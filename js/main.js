...
// ==========================================
const BG_MUSIC_URL = 'https://assets.mixkit.co/music/preview/mixkit-chill-bro-494.mp3';

// สร้างตัวเล่นเพลงให้อยู่ในระดับ Global เพื่อให้ detail.js เรียกใช้ได้
window.bgMusic = new Audio(BG_MUSIC_URL);
window.bgMusic.loop = true;

const savedTime = localStorage.getItem('bgMusicTime') || 0;
window.bgMusic.currentTime = parseFloat(savedTime);

// ฟังก์ชันปรับปรุงการแสดงผลของปุ่ม
function updateMusicButton(isPlaying) {
    const btn = document.getElementById('global-music-btn');
    if (!btn) return;
    btn.innerHTML = isPlaying ? '🔊 ปิดเพลง' : '🔇 เปิดเพลง';
    btn.style.backgroundColor = isPlaying ? '#28a745' : '#e50914';
}

// สร้างและแทรกปุ่มเปิด-ปิดเข้าไปในทุกหน้าอัตโนมัติ
document.addEventListener('DOMContentLoaded', () => {
    if (!document.getElementById('global-music-btn')) {
        ...
        if (isPlaying) {
            window.bgMusic.play().catch(() => {
                console.log("คลิกหน้าจอเพื่อเริ่มเล่นเพลงตามข้อกำหนดของเบราว์เซอร์");
            });
        }
